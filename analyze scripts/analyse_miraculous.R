...
process_transcript<-function(filepath){
  ...
  ep_df_values <- cbind(ep_title, ep_no, season, ep_per_season, air_date, ep_edge_density_value, ep_reciprocity_value, ep_diameter_value) #erweitern #todo:
...
colnames(dialogTable) <- c("From", "To", "Sentiment", "Text","Season", "Episode Overall")
colnames(lineTable) <- c("Character", "Sentiment", "Text", "Season", "Episode Overall")
